#include "buffer.h"
#include "util.hpp"
bool PCBuffer::get_resource_from_net(std::string url)
{

	return false;
}

bool PCBuffer::get_resource_from_buffer(std::string url)
{
	return false;
}
...
bool PCBuffer::add_resource(void* r, int data_size, std::string url)
{
	...
	if (data_size + current_size > MAX_SIZE) {
		log(format("try to add_resource failed due to insufficient space, url: %s data_size: %d ,max_size: %d", url.c_str(),data_size, MAX_SIZE));
		return false;
	}
